...
class DataSet(Data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        dia_iter = self.dataset[2][index]
        ins_dia_iter = self.dataset[3][index]
        cp_ix_iter = self.dataset[4][index]
        ans_iter = self.dataset[5][index]

        return que_iter, opt_iter, dia_iter, ins_dia_iter, cp_ix_iter, ans_iter
    ...
    def _get_dq_list(self, ques_path):
        que_list = [name for name in os.listdir(ques_path) if
                    os.path.isdir(os.path.join(ques_path, name)) and name.startswith('DQ')]
        que_list.sort()
        return que_list

    # get the list of instructional diagrams
    def _get_ins_dia_list(self, lesson_path):
        ins_dia_list = [name for name in os.listdir(lesson_path) if
                        os.path.isdir(os.path.join(lesson_path, name)) and name.startswith('DD')]
        ins_dia_list.sort()
        return ins_dia_list
    ...
    def _load_dataset(self, split):
        ...
        for lesson in lesson_list:
            ...
            dq_list = self._get_dq_list(lesson_path)
            ins_dia_list = self._get_ins_dia_list(lesson_path)
            ins_dia_per_lesson = []
            num_ins_dia = len(ins_dia_list)

            for index, ins_dia in enumerate(ins_dia_list):
                single_ins_dia = self._proc_dia(os.path.join(lesson_path, ins_dia))
                ins_dia_per_lesson.append(single_ins_dia)

                if index + 1 == self.cfgs.max_ins_dia:
                    break

                # pad instructional diagram per lesson to 5
                if num_ins_dia < self.cfgs.max_ins_dia and index + 1 == num_ins_dia:
                    difference = self.cfgs.max_ins_dia - index - 1
                    for i in range(difference):
                        single_ins_dia = np.zeros((3, self.cfgs.input_size, self.cfgs.input_size), dtype='float32')
                        ins_dia_per_lesson.append(single_ins_dia)

            for dq in dq_list:
                dq_path = os.path.join(lesson_path, dq)
                que, opt = self._proc_que_opt(dq_path)
                dia_per_que = self._proc_dia(dq_path)
                cp_per_que = self._proc_cp(dq_path)
                ans_of_que = self._proc_ans(dq_path)

                que_iter.append(que)
                opt_iter.append(opt)
                dia_iter.append(dia_per_que)
                ins_dia_iter.append(ins_dia_per_lesson)
        ...
        return torch.from_numpy(np.array(que_iter)), \
               ...
               torch.from_numpy(np.array(dia_iter)), \
               torch.from_numpy(np.array(ins_dia_iter)), \
    ...
    def _proc_dia(self, dia_path):
        dia_name = self._get_dia(dia_path)
        dia = Image.open(os.path.join(dia_path, dia_name)).convert('RGB')  # RGBA -> RGB
        dia = dia.resize((self.cfgs.input_size, self.cfgs.input_size), Image.BILINEAR)
        dia = transforms.ToTensor()(dia)
        dia = np.array(dia)
        return dia

    # process a question with its corresponding options.
    def _proc_que_opt(self, dq_path):
        opt_ix = np.zeros((self.cfgs.max_dq_ans, self.cfgs.max_opt_token), np.int64)
        que_ix = np.zeros(self.cfgs.max_que_token, np.int64)
        opt_name = 'a'

        with open(os.path.join(dq_path, 'Question.txt'), 'r') as f:
            que = f.read()
        ...
        counter = 0
        while os.path.exists(os.path.join(dq_path, opt_name) + '.txt'):
            with open(os.path.join(dq_path, opt_name) + '.txt', 'r') as f:
                opt = f.read()
    ...
    def _proc_ans(self, dq_path):
        ans_index = np.zeros(self.cfgs.max_dq_ans, np.int64)

        with open(os.path.join(dq_path, 'correct_answer.txt'), 'r') as f:
    ...
    def _token_to_index(self):
        ...
        for split in split_list:
            ...
            for lesson in tqdm(lesson_list):
                ...
                questions = self._get_dq_list(os.path.join(lesson_path, lesson))
