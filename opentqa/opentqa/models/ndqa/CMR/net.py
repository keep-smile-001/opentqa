...
class Net(nn.Module):
    def __init__(self, cfgs, pretrained_emb, token_size):
        ...
        self.encode_lang2 = nn.GRU(
            input_size=cfgs.word_emb_size,
            hidden_size=cfgs.hidden_size*2,
            batch_first=True,
        )
    ...
    def forward(self, que_ix, opt_ix, cp_ix):
        ...
        batch_size, img_num, obj_num, feat_size =  cp_feat.size()[0],1 ,cp_feat[1] , cp_feat.size()[2]
        #print("-----------------------------\n",cp_feat.size())
        assert img_num == 1 and feat_size == 2048
        cp_feat = cp_feat.reshape(batch_size , -1, feat_size)

        output_lang, output_img, output_cross = self.bert_encoder(sents=que_feat,feats=cp_feat)
       ...
       # #print(F.normalize(cross1,p=2,dim=-1).size())
        ...
        cross_logit = cross_logit.repeat(1, self.cfgs.max_ndq_ans).reshape(batch_size, self.cfgs.max_ndq_ans, -1)
