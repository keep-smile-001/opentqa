...
from opentqa.models.dqa.xtqa.layer import SimCLR, FlattenAtt, MLP, BAN, weight_norm
...
class Net(nn.Module):
    def __init__(self, cfgs, pretrained_emb, token_size):
        ...
        self.score_lang = MLP(
            dims=[cfgs.hidden_size, cfgs.flat_mlp_size, 1],
            act='',
        ...
        layers = [
            weight_norm(nn.Linear(cfgs.hidden_size * 9, cfgs.flat_out_size), dim=None),
            nn.ReLU(),
    ...
    def forward(self, que_ix, opt_ix, dia, ins_dia, cp_ix):
        ...
        evi_feat = evi_feat.repeat(1, self.cfgs.max_dq_ans).reshape(batch_size, self.cfgs.max_dq_ans, -1)

        # repeat flattened question features
        flattened_que_feat = flattened_que_feat.repeat(1, self.cfgs.max_dq_ans).reshape(batch_size, self.cfgs.max_dq_ans, -1)

        # use ban to fuse features of questions and diagrams
        fuse_que_dia = self.backbone(que_feat, dia_feat)
        fuse_que_dia = fuse_que_dia.mean(1).repeat(1, self.cfgs.max_dq_ans).reshape(batch_size, self.cfgs.max_dq_ans, -1)
        ...
        dia_feat = dia_feat.repeat(1, self.cfgs.max_dq_ans).reshape(batch_size, self.cfgs.max_dq_ans, -1)
