...
# ---version 1 for HMFN---
# ------------------------
class HMFN_V1(nn.Module):
    # HMFN: Hierarchical Multi-source Fusion Networks for Textbook Question Answering
    def __init__(self, cfgs):
        super(HMFN_V1, self).__init__()
        self.ban_qd = BAN(cfgs)
        self.ban_cf = BAN(cfgs)

    def forward(self, que_feat, dia_feat, cp_feat):
        batch_size = dia_feat.shape[0]
        feat_dim = dia_feat.shape[-1]

        dia_feat = dia_feat.reshape(batch_size, -1, feat_dim)
        qo_dia_feat = self.ban_qd(que_feat, dia_feat)  # use ban to fuse que_feat and dia_feat
        f = self.ban_cf(qo_dia_feat, cp_feat)  # use ban to fuse the above feature and cp_feat
        return f
        # return qo_dia_feat
